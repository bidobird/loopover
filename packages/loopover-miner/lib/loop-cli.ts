...
export type RunLoopOptions = {
  ...
  ) => Promise<{ spec: Record<string, unknown>; source: string; warnings: string[] }>;
  checkMinerKillSwitch?: (input?: {
    env?: Record<string, string | undefined>;
    repoPaused?: boolean;
  }) => { scope: "global" | "repo" | "none"; active: boolean };
  evaluateRunLoopBoundaryGate?: (
    input: unknown,
    options?: unknown,
  ) => { verdict: { reason: string }; canClaimNext: boolean };
  pollPrDisposition?: (
    repoFullName: string,
    prNumber: number,
    options?: PollPrDispositionOptions,
  ) => Promise<{ state: "open" | "closed"; merged: boolean; closedAt: string | null; attempts: number }>;
  pollCheckRuns?: (
    repoFullName: string,
    prNumber: number,
    options?: PollCheckRunsOptions,
  ) => Promise<{ conclusion: CheckRunConclusion; checks: unknown[]; headSha: string; attempts: number }>;
...
/** Fields only the `attempt_*` result arm carries (plus attempt-runner's kill-switch `abandonReason`, which the
 *  result union never declares) -- the loop has always read them optionally off whatever arm arrived, via `?.`
 *  with a `??` fallback, so the widened view just states that long-standing access pattern. */
type LoopAttemptResultView = AttemptCliResult &
  Partial<{ totalCostUsd: number; totalTurnsUsed: number; abandonReason: string; execResult: unknown }>;
...
function discoverArgv(parsed: { targets: string[]; search: string | null }): string[] {
...
export async function runLoop(args: string[], options: RunLoopOptions = {}): Promise<number> {
  ...
  const resolveAmsPolicyFn = (options.resolveAmsPolicy ?? resolveAmsPolicy) as NonNullable<
    RunLoopOptions["resolveAmsPolicy"]
  >;
  const checkKillSwitchFn = (options.checkMinerKillSwitch ?? checkMinerKillSwitch) as NonNullable<
    RunLoopOptions["checkMinerKillSwitch"]
  >;
  const evaluateBoundaryGateFn = (options.evaluateRunLoopBoundaryGate ?? evaluateRunLoopBoundaryGate) as NonNullable<
    RunLoopOptions["evaluateRunLoopBoundaryGate"]
  >;
  const pollPrDispositionFn = (options.pollPrDisposition ?? pollPrDisposition) as NonNullable<
    RunLoopOptions["pollPrDisposition"]
  >;
  const pollCheckRunsFn = (options.pollCheckRuns ?? pollCheckRuns) as NonNullable<RunLoopOptions["pollCheckRuns"]>;
  const recordPrOutcomeSnapshotFn = (options.recordPrOutcomeSnapshot ?? recordPrOutcomeSnapshot) as NonNullable<
    RunLoopOptions["recordPrOutcomeSnapshot"]
  >;
  const buildLoopClosureSummaryFn = (options.buildLoopClosureSummary ?? buildLoopClosureSummary) as NonNullable<
    RunLoopOptions["buildLoopClosureSummary"]
  >;
  const attemptLoopReentryFn = (options.attemptLoopReentry ?? attemptLoopReentry) as NonNullable<
    RunLoopOptions["attemptLoopReentry"]
  >;
  ...
  try {
    ...
    while (haltReason === null && (parsed.maxCycles === undefined || cycleIndex < parsed.maxCycles)) {
      ...
      const boundary = evaluateBoundaryGateFn(
        {
          ...
          markFailed: (repoFullName: string, identifier: string) =>
            portfolioQueue.markFailed(repoFullName, identifier, claimedApiBaseUrl),
        },
      ...
      let lastResult: AttemptCliResult | null = null;
      const attemptArgv = [
        claimed.repoFullName,
        String(issueNumber),
        "--miner-login",
      ...
      usage = {
        ...
        budgetSpent: usage.budgetSpent + ((lastResult as LoopAttemptResultView | null)?.totalCostUsd ?? 0),
        turnsTaken: usage.turnsTaken + ((lastResult as LoopAttemptResultView | null)?.totalTurnsUsed ?? 0),
        elapsedMs: usage.elapsedMs + cycleElapsedMs,
      };
      governorState.saveCapUsage(usage);

      const attemptOutcome = (lastResult as LoopAttemptResultView | null)?.outcome ?? "attempt_error";
      ...
      const killSwitchAbandon = (lastResult as LoopAttemptResultView | null)?.abandonReason === "kill_switch_engaged";
      ...
      if (submitted) {
        prNumber = parsePrNumberFromExecResult(
          (lastResult as LoopAttemptResultView | null)?.execResult as Parameters<typeof parsePrNumberFromExecResult>[0],
          claimed.repoFullName,
        );
        if (prNumber !== null) {
