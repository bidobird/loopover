...
function parseRepoTarget(value: string | undefined): string | null {
...
export function parseAttemptArgs(args: string[]): ParsedAttemptArgs {
  const options: { json: boolean; minerLogin: string | null; base: string; live: boolean; dryRun: boolean } = {
    json: false,
    minerLogin: null,
    base: "main",
    live: false,
...
export async function runAttempt(args: string[], options: RunAttemptOptions = {}): Promise<number> {
  ...
  let worktreeResult: (Awaited<ReturnType<typeof prepareAttemptWorktree>> & { attemptOk?: boolean }) | null = null;
  ...
  try {
    ...
    } as Parameters<typeof fetchSelfReviewContext>[1]);
    ...
    const codingTaskSpec = buildTaskSpec({
      ...
      claimLedger: claimLedger as unknown as Parameters<typeof buildCodingTaskSpec>[0]["claimLedger"],
    ...
    const finalResult = {
      outcome: `attempt_${result.outcome}` as const,
      repoFullName: parsed.repoFullName,
      issueNumber: parsed.issueNumber,
      minerLogin: parsed.minerLogin,
      base: parsed.base,
