...
import type { EnqueueRankedDiscoveryOptions, EnqueueRankedDiscoverySummary } from "./portfolio-discovery.js";
...
export type RunDiscoverOptions = {
  ...
  resolveContributionProfiles?: (
    ...
    ctx: { githubToken?: string; apiBaseUrl?: string | undefined; nowMs?: number | undefined },
  ) => Promise<Map<string, ContributionProfile>>;
};
...
export async function runDiscover(args: string[], options: RunDiscoverOptions = {}): Promise<number> {
  ...
  try {
    ...
    const enqueueSummary = enqueue(rankedSummary.issues, { queueStore: portfolioQueue, apiBaseUrl } as EnqueueRankedDiscoveryOptions);
